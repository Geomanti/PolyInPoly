...
def ShapeToViewContour(pointlist):
	x = []
	y = []
...
def ShapeToView(pointlist):
	...
	#x_2d, y_2d = np.meshgrid(x, y)
	plt.fill(x, y)
#Funtion to find intersections with second poly and creating transform vectors for scaling and also write intersection as index of poly 1
def InterPoints (poly1, poly2, interior):
	...
	for index1 in range(len(poly1)):
		...
		pointsmax = list()
		trvector = list()
		#print('first cycle counter ' + str(index1))
		for index2 in range(len(poly2)):
			...
			if (float(poly1[index1][0]) == float(poly2[index2][0])) and (float(poly1[index1][1]) == float(poly2[index2][1])):
				...
				intersect = 1
				#print('intersection ' + str(index2))
				#Cheking previous point for intersection
			if (float(poly1[minusindex][0]) == float(poly2[index2][0])) and (float(poly1[minusindex][1]) == float(poly2[index2][1])):
				intermin = 1
				pointsmin.append(poly1[minusindex])
				#print('intersection minpoint ' + str(index2))
			#Cheking next point for intersection
			if (float(poly1[plusindex][0]) == float(poly2[index2][0])) and (float(poly1[plusindex][1]) == float(poly2[index2][1])):
				intermax = 1
				pointsmax.append(poly1[plusindex])
				#print('intersection maxpoint ' + str(index2))
		if ((intermin == 1) and (intermax == 1)) and (intersect == 1):
			...
			if polygon1.contains(checkpoint):
				...
				'''
				if (poly1[index1] == poly1[0]) or (poly1[index1] == poly1[-1]):
					x = trvector[0] / 2
					y = trvector[1] / 2
					trvector = list()
					trvector.append(x)
				...
				#print('vector from midpoint ' + str(index1))
				#print(trvector)
			else:
				if interior == 0:
					trvector = Vec2pt(poly1[index1], midpoint)
				...
				if (poly1[index1] == poly1[0]) or (poly1[index1] == poly1[-1]):
					...
					'''
				movevector.append(trvector)
				#print('vector from midpoint reversed' + str(index1))
				#print(trvector)
		if ((intermin == 1) or (intermax == 1)) and (intersect == 1) and (stopper == 0):
			if intermax == 1:
				if interior == 0:
				...
				if (poly1[index1] == poly1[0]) or (poly1[index1] == poly1[-1]):
					...
					'''
				movevector.append(trvector)
				#print('vector from minpoint ' + str(index1))
				#print(trvector)
			else:
				if interior == 0:
				...
				if (poly1[index1] == poly1[0]) or (poly1[index1] == poly1[-1]):
					...
					'''
				movevector.append(trvector)
				#print('vector from maxpoint ' + str(index1))
				#print(trvector)
		#print(movevector)
...
def ScaleVtoFitA (points, pointstransformed, vectorlist, refpoly, value):
	...
	#if ((value / polygon1.area) >= 0.75) or ((value / polygon1.area) <= 0.3):
		#scale = value / polygon1.area * 0.1179
	#else:
	#	scale = value / polygon1.area * 0.125
	scalar = (refpoly1.area + value - polygonoptimized.area) / (polygontransformed.area - polygonoptimized.area)
	newvectorlist = list()
	newvector = list()
	for vector in vectorlist:
		x = vector[0] * scalar
		...
		for index1 in range(len(rings)):
			...
			for index2 in range(len(poly2)):
				...
				if index1-1 == -1:
					...
					minusindex = index1-1
				if (float(rings[index1][0]) == float(poly2[index2][0])) and (float(rings[index1][1]) == float(poly2[index2][1])):
					intersect = 1
					#print('intersect ' + str(index1))
				if (float(rings[minusindex][0]) == float(poly2[index2][0])) and (float(rings[minusindex][1]) == float(poly2[index2][1])):
					intermin = 1
					pointsmin.append(rings[minusindex])
					#print('intersect min ' + str(index1))
				if (float(rings[plusindex][0]) == float(poly2[index2][0])) and (float(rings[plusindex][1]) == float(poly2[index2][1])):
					intermax = 1
					pointsmax.append(rings[plusindex])
					#print('intersect max ' + str(index1))
				if ((intermin == 1) and (intermax == 1)) and (intersect == 1) and (stopper == 0):
					stopper = 1
					#print('midpoint ' + str(index1))
					pmin = pointsmin[0]
					pmax = pointsmax[0]
					midpoint = list()
					midpoint = MidPoint(pmin, pmax)
					...
					if polygon1.contains(checkpoint):
						#print('midpoint in poly ' + str(index1))
						if inter == 0:
							pass
						else:
							#print('removing point ' + str(index1))
							skip = True
					else:
						#print('removing point ' + str(index1))
						skip = True
